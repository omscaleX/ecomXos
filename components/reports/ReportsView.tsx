...
import type { BrandId, RangeInput } from "@/types";
...
export function ReportsView() {
  ...
  const { range, setRange } = usePeriod("7d");
  ...
  return (
    <div className="space-y-6">
      ...
      {status !== "done" ? (
        ...
        <ReportPreview brandIds={selected.map((b) => b.id)} range={range} periodLabel={describeRange(range)} from={dates.from} to={dates.to} generatedBy={currentUser.name} />
      )}
    </div>
  );
}

function ReportPreview({ brandIds, range, periodLabel, from, to, generatedBy }: { brandIds: BrandId[]; range: RangeInput; periodLabel: string; from: string; to: string; generatedBy: string }) {
      ...
      <CardHeader>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <CardDescription>
              {periodLabel} · {formatLongDate(from)} – {formatLongDate(to)} · generated by {generatedBy} · <Badge variant="neutral">Demo Data</Badge>
            </CardDescription>
          </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {portfolios.map((p) => (
              <dl key={p.currency} className="grid grid-cols-2 gap-2 rounded-md border p-3 text-sm sm:grid-cols-4">
                ...
                <div><dt className="text-[11px] text-muted-foreground">Returned</dt><dd className="tabular font-semibold text-red-600">-{formatCurrency(p.returnedAmount, p.currency)}</dd></div>
                <div><dt className="text-[11px] text-muted-foreground">Meta reported sales</dt><dd className="tabular font-semibold text-muted-foreground">{formatCurrency(p.metaReportedSales, p.currency)}</dd></div>
              </dl>
            ))}
          </div>
        </section>
